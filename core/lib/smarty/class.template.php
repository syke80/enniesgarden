...
class template {
	...
	var $template_default_dir	= "templates";	// where the templates are to be found
	...
	// if doesn't exists in the template_dir, returns template_default_dir
	function getfilename($file) {
		return file_exists($this->template_dir.$file) ? $this->template_dir.$file : $this->template_default_dir.$file;
	}
	...
	function config_load($file, $section_name = null, $var_name = null) {
		...
		$this->compile_dir = $this->_get_dir($this->compile_dir);
		$name = md5($this->getfilename($file).$section_name.$var_name).'.php';
	...
	function _is_cached($file, $cache_id) {
		...
		if (file_exists($this->_cache_dir.$name) && (((time() - filemtime($this->_cache_dir.$name)) < $this->cache_lifetime) || $this->cache_lifetime == -1) && (filemtime($this->_cache_dir.$name) > filemtime($this->getfilename($file)))) {
	...
	function _fetch_compile($file) {
		...
		if (!$this->force_compile && file_exists($this->compile_dir.'c_'.$name) && (filemtime($this->compile_dir.'c_'.$name) > filemtime($this->getfilename($file)))) {
			ob_start();
			include($this->compile_dir.'c_'.$name);
			$output = ob_get_contents();
			ob_end_clean();
		...
		if ($this->template_exists($file)) {
			$f = fopen($this->getfilename($file), "r");
			$size = filesize($this->getfilename($file));
			if ($size > 0) {
				$file_contents = fread($f, filesize($this->getfilename($file)));
			} else {
				$file_contents = "";
			}
			$this->_file = $file;
	...
	function _destroy_dir($file, $id, $dir) {
		if ($file == null && $id == null) {
			...
			if ($id == null) {
				$this->template_dir = $this->_get_dir($this->template_dir);
				$this->template_default_dir = $this->_get_dir($this->template_default_dir);
				@unlink($dir.md5($this->getfilename($file)).'.php');
			} else {
				$_args = "";
