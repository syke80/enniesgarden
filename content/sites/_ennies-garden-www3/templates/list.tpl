...
<li>
	<span class="name">
		<a href="<% $siteconfig[site_url] %>/<% $product[permalink] %>">
			<% $product[name] %>
		</a>
	</span>
...
<li>
	...
	<a class="photo" href="<% $siteconfig[site_url] %>/<% $pack[permalink] %>">
		<img src="<% imgresize file='content/files/pack/original/'.$pack[photo_filename] resized_filename='content/files/pack/resized/'.$pack[id_pack]|id2dir.'/'.$pack[permalink].'_'.$pack[category_permalink].'_'.$pack[shop_permalink].'_'.$pack[id_photo].'_100x160_crop' width=100 height=160 type='crop' ratio=$pixel_ratio %>" alt="pack photo" />
	</a>
	<span class="price"><% $pack[price]|number_format:2:'.':' ' %> Ft</span>
	<div class="cart">
...
onLoadFunctions.push( function() {
	$(".btn_cart").click( function() {
		...
		$.ajax({
			type: "POST",
			url: "<% $siteconfig[site_url] %>/basket/",
			data: {
			...
			success: function(data) {
				...
				if (data.error == "") {
					...
					$("#cart").load("<% $siteconfig[site_url] %>/basket/gadget/<% $lang_iso %>/");
				}
				else {
